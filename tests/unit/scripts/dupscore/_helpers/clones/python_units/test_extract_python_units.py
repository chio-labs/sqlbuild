...
@pytest.mark.parametrize(
    ...
    [
        ...
        PythonKeyTestCase(
            ...
            left_source="def f(orders):\n    return orders.load(1)\n",
            right_source="def f(orders):\n    return orders.save(1)\n",
            expected_same_concrete=False,
            expected_same_normalized=False,
        ),
        PythonKeyTestCase(
            description="call receivers and plain attribute reads stay abstracted",
            left_source="def f(orders):\n    return orders.total(orders.count)\n",
            right_source="def f(items):\n    return items.total(items.size)\n",
            expected_same_concrete=False,
            expected_same_normalized=True,
        ),
        PythonKeyTestCase(
            description="method calls after non-ascii text keep their call target",
            left_source='def f(a):\n    label = "caf\u00e9"; return label.upper()\n',
            right_source='def f(a):\n    label = "caf\u00e9"; return label.lower()\n',
            expected_same_concrete=False,
            expected_same_normalized=False,
        ),
        PythonKeyTestCase(
            description="non-ascii literals before the same method call stay renamed",
            left_source='def f(a):\n    label = "caf\u00e9"; return label.upper()\n',
            right_source='def f(b):\n    name = "cr\u00e8me"; return name.upper()\n',
            expected_same_concrete=False,
            expected_same_normalized=True,
        ),
        PythonKeyTestCase(
