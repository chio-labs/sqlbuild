...
@pytest.mark.parametrize(
    ...
    [
        ...
        MicrobatchCoverageProjectionTestCase(
            description="duplicate completion facts do not duplicate projected coverage",
            events=(
                completion_event(event_id="duplicate", start="0", end="2"),
                completion_event(event_id="duplicate", start="0", end="2"),
            ),
            expected_intervals=expected_integer_intervals()[:2],
            cursor_type="integer",
        ...
        MicrobatchCoverageProjectionTestCase(
            description="wide historical completion splits across smaller expected batches",
            events=(completion_event(event_id="wide", start="0", end="3"),),
            expected_intervals=expected_integer_intervals(),
            cursor_type="integer",
            expected_projected_intervals=(
                ("0", "1", "F2", MicrobatchFingerprintStatus.KNOWN.value),
            ...
            expected_known_missing=(),
            expected_unaccounted=(),
            expected_contiguous_frontier="3",
        ),
        MicrobatchCoverageProjectionTestCase(
            description="smaller historical completions merge into a larger expected batch",
            events=(
                completion_event(event_id="first", start="0", end="1"),
                completion_event(event_id="second", start="1", end="2"),
                completion_event(event_id="third", start="2", end="3"),
            ),
            expected_intervals=(MicrobatchInterval(start="0", end="3"),),
            cursor_type="integer",
            expected_projected_intervals=(
                ("0", "1", "F2", MicrobatchFingerprintStatus.KNOWN.value),
                ("1", "2", "F2", MicrobatchFingerprintStatus.KNOWN.value),
