...
from rules.custom import check
...
CASES = [
    Wrapper(rule_case=RuleCase(description="one", source="", expected_finding_count=0)),
    Wrapper(rule_case=RuleCase(description="two", source="", expected_finding_count=0)),
]
...
CASES = [
    MissingWrapper(
        rule_case=RuleCase(description="missing", source="", expected_finding_count=0)
    )
]
...
""",
        ...
        CustomRuleEvidenceTestCase(
            ...
            test_source="""from sqlbuild.rules.testing import RuleCase, evaluate_rule
from rules.custom import check

CASES = [RuleCase(description="one", source="", expected_finding_count=0)]
...
class TestEvidence:
    def test_exact(self):
        evaluate_rule(
            rule=check,
            test_case=RuleCase(description="method", source="", expected_finding_count=0),
        )
""",
            expected_count=1,
        ),
...
def outer():
    def test_exact():
        evaluate_rule(
            rule=check,
            test_case=RuleCase(description="nested", source="", expected_finding_count=0),
        )
""",
            expected_count=0,
        ),
...
def test_exact():
    evaluate_rule(
        rule=check,
        test_case=RuleCase(description="skipped", source="", expected_finding_count=0),
    )
""",
            expected_count=0,
        ),
...
def test_exact():
    evaluate_rule(
        rule=check,
        test_case=RuleCase(description="active", source="", expected_finding_count=0),
    )
""",
            expected_count=1,
        ),
...
def test_given_rule_and_test_source_when_collecting_evidence_then_only_exact_static_cases_count(
    ...
    rule: Rule = load_custom_rule(root=tmp_path, configured_path=configured_path)
...
def test_exact():
    evaluate_rule(
        rule=check,
        test_case=RuleCase(description="exact", source="", expected_finding_count=0),
    )
""",
            expected_count=0,
        )
...
def test_given_collectable_rule_evidence_when_evaluating_then_coverage_fault_clears(
    ...
    assert len(result.findings) == test_case.expected_count
...
def test_given_rewritten_rule_when_loading_again_then_evidence_does_not_leak_between_loads(
    ...
    test_path.parent.mkdir()
    test_path.write_text(test_case.test_source, encoding="utf-8")
    first_rule: Rule = load_custom_rule(root=tmp_path, configured_path=configured_path)
    ...
    second_rule: Rule = load_custom_rule(root=tmp_path, configured_path=configured_path)
...
def test_given_imported_helper_change_when_fingerprinting_then_fingerprint_changes(
    ...
    configured_path: Path = write_rule(
        root=tmp_path,
        body="del model\n    return []",
        module_import="from .helpers import VALUE",
    )
    helper: Path = tmp_path / "rules" / "helpers.py"
...
@pytest.mark.parametrize(
    ...
    [
        CustomRuleEvidenceTestCase(
            ...
            test_source="duplicate rule codes: XSQBRT101",
            expected_count=1,
        )
    ],
    ids=lambda case: case.description,
...
def test_given_duplicate_source_codes_when_building_catalogue_then_rejects_duplicates(
    ...
    with pytest.raises(RulesError, match=test_case.test_source) as raised:
        build_catalogue(config=config, project_dir=tmp_path)
    assert str(raised.value).count("XSQBRT101") == test_case.expected_count
...
@pytest.mark.parametrize(
    ...
    [
        CustomRuleEvidenceTestCase(
            description="new custom rule evidence invalidates a cached coverage fault",
            test_source="""from sqlbuild.rules.testing import RuleCase, evaluate_rule
from rules.custom import check
...
""",
            expected_count=1,
            expected_fault_codes_before=("SQBRTEST301",),
            expected_fault_codes_after=(),
        )
    ],
    ids=lambda case: case.description,
)
def test_given_cached_coverage_fault_when_evidence_changes_then_cache_is_invalidated(
    tmp_path: Path,
    test_case: CustomRuleEvidenceTestCase,
) -> None:
    project: CompiledProject
    ...
    before: RulesResult = evaluate(project=project, config=config, project_dir=tmp_path)
    test_path: Path = tmp_path / test_case.test_path
    test_path.parent.mkdir(parents=True, exist_ok=True)
    test_path.write_text(test_case.test_source, encoding="utf-8")

    after: RulesResult = evaluate(project=project, config=config, project_dir=tmp_path)

    assert tuple(fault.code for fault in before.findings) == test_case.expected_fault_codes_before
    assert tuple(fault.code for fault in after.findings) == test_case.expected_fault_codes_after
