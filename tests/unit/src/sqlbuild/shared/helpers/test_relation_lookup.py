...
RELATION_LOOKUP_TEST_CASES: list[RelationLookupTestCase] = [
    RelationLookupTestCase(
        ...
        expected_queried_schema_groups=(("marts", "staging"),),
    ),
    RelationLookupTestCase(
        ...
        expected_list_relations_calls=1,
        expected_queried_schema_groups=(("marts",),),
    ),
    RelationLookupTestCase(
        description="no concrete schemas issues no warehouse query",
        warehouse_relations=((None, "fact_orders", True),),
        ...
        expected_exists=False,
        expected_is_transient=False,
        expected_list_relations_calls=0,
        expected_queried_schema_groups=(),
    ),
]
...
def test_given_locations_when_building_relation_lookup_then_batches_and_answers_queries(
    ...
    assert (
        lookup.exists(schema=test_case.probe_schema, name=test_case.probe_name)
        == test_case.expected_exists
    )
    assert (
        lookup.is_transient(schema=test_case.probe_schema, name=test_case.probe_name)
        == test_case.expected_is_transient
    )
    assert len(adapter.list_relations_calls) == test_case.expected_list_relations_calls
    assert (
        tuple(tuple(call) for call in adapter.list_relations_calls)
        == test_case.expected_queried_schema_groups
    )
