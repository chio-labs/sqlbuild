...
@pytest.mark.parametrize(
    ...
    (
        ...
        KataEvaluationTestCase(
            ...
            model_name="commerce__stg__orders__web_archive",
            relative_path="models/staging/commerce__stg__orders__web_archive.sql",
            sql="WITH prices AS (SELECT id FROM source_prices) SELECT id FROM prices",
            config_values={"materialized": "table", "contract": "enforced"},
            select=("SQBKR201",),
            expected_codes=("SQBKR201",),
            kata_config=KataConfig(retired_source_tokens={"web_archive": "legacy_partner_archive"}),
        ),
        KataEvaluationTestCase(
...
def test_given_join_shape_when_evaluating_join_rules_then_enforces_hard_policy(
    ...
    model_path: Path = tmp_path / "models/mart/commerce__mart__prices.sql"
