...
from sqlbuild.policy import RuleCase, RuleResult, evaluate_rule
...
from sqlbuild.policy_engine.models import PolicyCacheConfig, PolicyConfig, PolicyResult
from sqlbuild.policy_engine.types import RuleOptionValue
from tests.unit.src.sqlbuild.policy_engine.main.evaluate.helpers import build_project
from tests.unit.src.sqlbuild.policy_engine.main.evaluate_rule._test_types import (
    EvaluateRuleParityTestCase,
    EvaluateRuleTestCase,
)
from tests.unit.src.sqlbuild.policy_engine.main.evaluate_rule.helpers import required_domain
...
def test_given_same_custom_rule_when_using_public_and_native_paths_then_faults_have_full_parity(
    ...
    rule_path: Path = tmp_path / "policy" / "rules" / "custom.py"
    ...
    config: PolicyConfig = PolicyConfig(
        ...
        rule_options={
            "XSQBPD001": cast("dict[str, RuleOptionValue]", rule_case.config),
        },
        rule_paths=("policy/rules/custom.py",),
        cache=PolicyCacheConfig(enabled=False),
    )

    native_result: PolicyResult = evaluate(
        project=project,
        config=config,
        project_dir=tmp_path,
    )
    repeated_native_result: PolicyResult = evaluate(
        project=project,
        config=config,
        project_dir=tmp_path,
    )
