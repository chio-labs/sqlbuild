...
from sqlbuild.policy_engine.models import PolicyConfig, PolicyResult, RuleExemption, RuleIgnore
from tests.unit.src.sqlbuild.policy_engine.main.evaluate._test_types import PolicyBehaviorTestCase
...
def test_given_unchanged_builtin_inputs_when_evaluating_twice_then_second_run_hits_cache(
    ...
    first: PolicyResult = evaluate(project=project, config=config, project_dir=tmp_path)
...
def test_given_authored_sql_changes_when_compiled_sql_is_same_then_cache_is_invalidated(
    ...
    config: PolicyConfig = PolicyConfig(select=("SQBPD102",))
    _ = evaluate(project=first_project, config=config, project_dir=tmp_path)

    result: PolicyResult = evaluate(project=bare_project, config=config, project_dir=tmp_path)
...
@pytest.mark.parametrize(
    ...
    [
        PolicyBehaviorTestCase(
            ...
            expected_error_pattern="stale policy exception",
        )
    ],
    ids=lambda case: case.description,
)
def test_given_stale_exact_exception_when_evaluating_then_raises_error(
    ...
    config: PolicyConfig = PolicyConfig(
        select=("SQBPS103",),
        rule_exceptions=(
            RuleExemption(rule="SQBPS103", path=relative_path, reason="Migration is tracked"),
        ),
    )
