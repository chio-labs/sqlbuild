...
from sqlbuild.policy_engine._helpers.engine.custom_rule_evidence import (
    _RuleTestEvidence,
...
from tests.unit.src.sqlbuild.policy_engine._helpers.engine.helpers import (
    custom_rule_inputs,
    load_custom_rule,
    write_rule,
)
...
from policy.rules.custom import check
...
""",
        ...
        CustomRuleEvidenceTestCase(
            ...
            test_source="""from sqlbuild.policy import RuleCase, evaluate_rule
from policy.rules.custom import check
...
def test_given_rule_and_test_source_when_collecting_evidence_then_only_exact_static_cases_count(
    ...
    rule: PolicyRule = load_custom_rule(root=tmp_path, configured_path=configured_path)
...
def test_given_rewritten_rule_when_loading_again_then_evidence_does_not_leak_between_loads(
    ...
    test_path.parent.mkdir()
    test_path.write_text(test_case.test_source, encoding="utf-8")
    first_rule: PolicyRule = load_custom_rule(root=tmp_path, configured_path=configured_path)
    ...
    second_rule: PolicyRule = load_custom_rule(root=tmp_path, configured_path=configured_path)
...
def test_given_imported_helper_change_when_fingerprinting_then_fingerprint_changes(
    ...
    configured_path: Path = write_rule(
        root=tmp_path,
        body="del model\n    return []",
        module_import="from policy.helpers import VALUE",
    )
    helper: Path = tmp_path / "policy" / "helpers.py"
...
@pytest.mark.parametrize(
    ...
    [
        CustomRuleEvidenceTestCase(
            ...
            test_source="duplicate policy rule codes: XSQBPT101",
            expected_count=1,
        )
    ],
    ids=lambda case: case.description,
...
def test_given_duplicate_source_codes_when_building_catalogue_then_rejects_duplicates(
    ...
    config: PolicyConfig = PolicyConfig(rule_paths=("policy/rules",))

    with pytest.raises(PolicyError, match=test_case.test_source) as raised:
        build_catalogue(config=config, project_dir=tmp_path)
    assert str(raised.value).count("XSQBPT101") == test_case.expected_count
...
""",
            expected_count=1,
            expected_fault_codes_before=("SQBPT301",),
...
def test_given_cached_coverage_fault_when_evidence_changes_then_cache_is_invalidated(
    ...
    before: PolicyResult = evaluate(project=project, config=config, project_dir=tmp_path)
    test_path: Path = tmp_path / test_case.test_path
    test_path.parent.mkdir(parents=True, exist_ok=True)
    test_path.write_text(test_case.test_source, encoding="utf-8")

    after: PolicyResult = evaluate(project=project, config=config, project_dir=tmp_path)

    assert tuple(fault.code for fault in before.faults) == test_case.expected_fault_codes_before
    assert tuple(fault.code for fault in after.faults) == test_case.expected_fault_codes_after
