...
@pytest.mark.parametrize(
    ...
    [
        SnowflakePermanentCopyTestCase(
            ...
            destination="analytics.mart.results",
            origin="analytics.mart.__sqb_staging__results",
            expected_sql=(
                "CREATE TABLE analytics.mart.results AS SELECT * FROM "
                "analytics.mart.__sqb_staging__results"
            ),
        )
    ],
    ids=lambda case: case.description,
