...
@pytest.mark.parametrize(
    ...
    [
        ...
        RunDespiteUnchangedPlanningTestCase(
            description="active hour duration marks root and downstream stale",
            run_despite_unchanged="12h",
            materialized="table",
            data_version="2026-06-10T18:00:00+00:00",
            value_kind=SourceFreshnessValueKind.TIMESTAMP.value,
            expected_root_model_names=frozenset({"rolling_orders"}),
            expected_stale_model_names=frozenset({"rolling_orders", "orders_mart"}),
        ),
        RunDespiteUnchangedPlanningTestCase(
            description="active minute duration marks root and downstream stale",
            run_despite_unchanged="90m",
            materialized="table",
            data_version="2026-06-10T23:00:00+00:00",
            value_kind=SourceFreshnessValueKind.TIMESTAMP.value,
            expected_root_model_names=frozenset({"rolling_orders"}),
            expected_stale_model_names=frozenset({"rolling_orders", "orders_mart"}),
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        RunDespiteUnchangedPlanningTestCase(
            description="unsupported second duration raises clear error",
            run_despite_unchanged="30s",
            materialized="table",
            data_version="2026-06-01T00:00:00+00:00",
            value_kind=SourceFreshnessValueKind.TIMESTAMP.value,
            expected_root_model_names=frozenset(),
