"""Postgres E2E tests for declarative dlt source loading."""
...
@pytest.mark.parametrize(
    ...
    [
        PostgresDltE2ETestCase(
            description="loads sqlite dlt source into postgres raw schema",
            expected_loaded_rows=((1, 10), (2, 20), (3, 30)),
            expected_model_rows=((2, 20), (3, 30)),
        )
    ],
    ids=["loads sqlite dlt source into postgres raw schema"],
)
def test_given_dlt_source_when_loading_to_postgres_then_table_is_materialized_and_queryable(
    tmp_path: Path,
    postgres_e2e_config: dict[str, object],
    test_case: PostgresDltE2ETestCase,
) -> None:
    schema_base: str = build_unique_schema_name(prefix="sqb_dlt_postgres")
    model_schema_name: str = f"{schema_base}_models"
    raw_schema_name: str = f"{schema_base}_raw"
    project_name: str = "dlt_postgres_project"
    source_db_path: Path = tmp_path / project_name / "source.db"
    try:
        ensure_postgres_schema_ready(schema_name=model_schema_name, config=postgres_e2e_config)
        ensure_postgres_schema_ready(schema_name=raw_schema_name, config=postgres_e2e_config)
        ...
        loaded_rows: tuple[tuple[object, ...], ...] = fetch_postgres_rows(
            config=postgres_e2e_config,
            sql=(
        ...
        cleanup_postgres_schema(schema_name=model_schema_name, config=postgres_e2e_config)
