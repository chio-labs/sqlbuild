...
from tests.e2e.src.sqlbuild.cli.commands.main.selection_staleness.helpers import (
    assert_dbt_selection_staleness_case,
    assert_native_selection_staleness_case,
    assert_selection_staleness_case,
    dbt_diamond_files,
    dbt_direct_files,
    dbt_mixed_parent_files,
    dbt_multi_hop_files,
    dbt_seed_parent_files,
...
_DBT_BASELINE_COMMAND: tuple[str, ...] = (
    "--no-color",
    "dbt",
    "build",
    "--select",
...
_DBT_FACT_ROWS_QUERY: str = (
    "SELECT order_id, amount_dollars FROM main.fact_orders ORDER BY order_id"
)
_NATIVE_STALENESS_LABEL: str = "selected model 'fact_orders' will build on"
_DBT_STALENESS_LABEL: str = "selected dbt model 'fact_orders' will build on"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        SelectionStalenessEngineE2ETestCase(
            description="dbt: direct changed model parent outside selection leaves child stale and warns",
            project_name="dbt_selection_staleness_direct_parent",
            graph="stg_orders -> fact_orders",
            runner=assert_dbt_selection_staleness_case,
            baseline_files=dbt_direct_files(
                amount_cents=100,
                fact_adjustment=0,
                leaf_materialization="table",
            ),
            mutated_files=dbt_direct_files(
                amount_cents=125,
                fact_adjustment=0,
                leaf_materialization="table",
            ),
            baseline_command=_DBT_BASELINE_COMMAND,
            exact_commands=(_DBT_EXACT_COMMAND,),
            repair_command=_DBT_REPAIR_COMMAND,
            expected_exact_stdout_fragments=(_DBT_STALENESS_LABEL, "- stg_orders"),
            unexpected_exact_stdout_fragments=("model    stg_orders",),
            expected_repair_stdout_fragments=(
                "Upstream changed",
                "stg_orders",
                "fact_orders",
            ),
            unexpected_repair_stdout_fragments=(_DBT_STALENESS_LABEL,),
            expected_rows_after_baseline=((1, 1.0),),
            expected_rows_after_exact_commands=(((1, 1.0),),),
            expected_rows_after_repair=((1, 1.25),),
            database_relative_path=_DBT_DATABASE_PATH,
            fact_rows_query=_DBT_FACT_ROWS_QUERY,
            notes="Exact leaf selection must not silently incorporate an unselected changed parent.",
        ),
        SelectionStalenessEngineE2ETestCase(
        ...
        SelectionStalenessEngineE2ETestCase(
            description="dbt: multi-hop changed model root outside selection leaves leaf stale and warns",
            project_name="dbt_selection_staleness_multi_hop",
            graph="raw_orders_model -> stg_orders -> fact_orders",
            runner=assert_dbt_selection_staleness_case,
            baseline_files=dbt_multi_hop_files(
                amount_cents=100,
                fact_adjustment=0,
                leaf_materialization="table",
            ),
            mutated_files=dbt_multi_hop_files(
                amount_cents=125,
                fact_adjustment=0,
                leaf_materialization="table",
            ),
            ...
            repair_command=_DBT_REPAIR_COMMAND,
            expected_exact_stdout_fragments=(
                _DBT_STALENESS_LABEL,
                "- raw_orders_model",
                "- stg_orders",
            ),
            unexpected_exact_stdout_fragments=(
                "model    raw_orders_model",
                "model    stg_orders",
            ),
            expected_repair_stdout_fragments=(
                "Upstream changed",
        ...
        SelectionStalenessEngineE2ETestCase(
            description="dbt: selected leaf own-change runs and still warns for unselected changed parent",
            project_name="dbt_selection_staleness_leaf_runs_and_warns",
            graph="stg_orders -> fact_orders",
            runner=assert_dbt_selection_staleness_case,
            baseline_files=dbt_direct_files(
                amount_cents=100,
        ...
        SelectionStalenessEngineE2ETestCase(
            description="dbt: selected root and leaf warn for unselected stale intermediate",
            project_name="dbt_selection_staleness_selected_root_leaf",
            graph="raw_orders_model -> stg_orders -> fact_orders",
            runner=assert_dbt_selection_staleness_case,
            baseline_files=dbt_multi_hop_files(
                amount_cents=100,
            ...
            unexpected_exact_stdout_fragments=("model    stg_orders",),
            expected_repair_stdout_fragments=("stg_orders", "fact_orders"),
            unexpected_repair_stdout_fragments=(_DBT_STALENESS_LABEL,),
            expected_rows_after_baseline=((1, 1.0),),
            expected_rows_after_exact_commands=(((1, 1.0),),),
            expected_rows_after_repair=((1, 1.25),),
        ...
        SelectionStalenessEngineE2ETestCase(
            description="dbt: mixed selected and unselected changed parents warn only for unselected",
            project_name="dbt_selection_staleness_mixed_parents",
            graph="selected_parent -> fact_orders, unselected_parent -> fact_orders",
            runner=assert_dbt_selection_staleness_case,
            baseline_files=dbt_mixed_parent_files(amount_cents=100, leaf_materialization="table"),
            mutated_files=dbt_mixed_parent_files(amount_cents=125, leaf_materialization="table"),
            baseline_command=_DBT_BASELINE_COMMAND,
            exact_commands=(
                (
                    "--no-color",
        ...
        SelectionStalenessEngineE2ETestCase(
            description="dbt: full-refresh selected leaf still warns for unselected changed parent",
            project_name="dbt_selection_staleness_full_refresh_leaf",
            graph="stg_orders -> fact_orders",
            runner=assert_dbt_selection_staleness_case,
            baseline_files=dbt_direct_files(
                amount_cents=100,
        ...
        SelectionStalenessEngineE2ETestCase(
            description="dbt: view selected leaf still warns for unselected changed parent",
            project_name="dbt_selection_staleness_view_leaf",
            graph="stg_orders -> fact_orders",
            runner=assert_dbt_selection_staleness_case,
            baseline_files=dbt_direct_files(
                amount_cents=100,
        ...
        SelectionStalenessEngineE2ETestCase(
            description="dbt: changed seed parent outside selection leaves child stale and warns",
            project_name="dbt_selection_staleness_seed_parent",
            graph="seed raw_orders -> stg_orders -> fact_orders",
            runner=assert_dbt_selection_staleness_case,
            baseline_files=dbt_seed_parent_files(
                amount_cents=100,
                fact_adjustment=0,
                leaf_materialization="table",
            ),
            mutated_files=dbt_seed_parent_files(
                amount_cents=125,
                fact_adjustment=0,
                leaf_materialization="table",
            ),
            ...
            exact_commands=(_DBT_EXACT_COMMAND,),
            repair_command=_DBT_REPAIR_COMMAND,
            expected_exact_stdout_fragments=(_DBT_STALENESS_LABEL, "- raw_orders"),
            unexpected_exact_stdout_fragments=("model    stg_orders",),
            expected_repair_stdout_fragments=("stg_orders", "fact_orders"),
            unexpected_repair_stdout_fragments=(_DBT_STALENESS_LABEL,),
            expected_rows_after_baseline=((1, 1.0),),
        ...
        SelectionStalenessEngineE2ETestCase(
            description="dbt: later unscoped build repairs stale downstream",
            project_name="dbt_selection_staleness_later_unscoped_repair",
            graph="stg_orders -> fact_orders",
            runner=assert_dbt_selection_staleness_case,
            baseline_files=dbt_direct_files(
                amount_cents=100,
            ...
            baseline_command=_DBT_BASELINE_COMMAND,
            exact_commands=(_DBT_EXACT_COMMAND,),
            repair_command=("--no-color", "dbt", "build"),
            expected_exact_stdout_fragments=(_DBT_STALENESS_LABEL,),
            unexpected_exact_stdout_fragments=(),
            expected_repair_stdout_fragments=(
                "Upstream changed",
                "stg_orders",
        ...
        SelectionStalenessEngineE2ETestCase(
            description="dbt: repeated exact selection preserves stale warning and warehouse state",
            project_name="dbt_selection_staleness_repeated_exact",
            graph="stg_orders -> fact_orders",
            runner=assert_dbt_selection_staleness_case,
            baseline_files=dbt_direct_files(
                amount_cents=100,
            ...
            baseline_command=_DBT_BASELINE_COMMAND,
            exact_commands=(_DBT_EXACT_COMMAND, _DBT_EXACT_COMMAND),
            repair_command=_DBT_REPAIR_COMMAND,
            expected_exact_stdout_fragments=(_DBT_STALENESS_LABEL, "- stg_orders"),
            unexpected_exact_stdout_fragments=("model    stg_orders",),
            expected_repair_stdout_fragments=(
        ...
        SelectionStalenessEngineE2ETestCase(
            description="dbt: diamond graph reports both stale branches",
            project_name="dbt_selection_staleness_diamond",
            graph="raw_orders_model -> stg_orders_a,b -> fact_orders",
            runner=assert_dbt_selection_staleness_case,
            baseline_files=dbt_diamond_files(amount_cents=100, leaf_materialization="table"),
            mutated_files=dbt_diamond_files(amount_cents=125, leaf_materialization="table"),
            baseline_command=_DBT_BASELINE_COMMAND,
            exact_commands=(_DBT_EXACT_COMMAND,),
            repair_command=_DBT_REPAIR_COMMAND,
            expected_exact_stdout_fragments=(
