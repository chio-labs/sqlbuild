...
@pytest.mark.parametrize(
    ...
    [
        ...
        PostgresRowDiffTestCase(
            ...
            right_sql=(
                "SELECT 1 AS id, 'a' AS val UNION ALL SELECT 2, 'x' UNION ALL SELECT 4, 'd'"
            ),
            unique_key=("id",),
            expected_result=RowDiffResult(
                left_count=3,
...
def test_given_two_relations_when_diffing_rows_then_postgres_returns_diff_counts(
    ...
    adapter.execute(connection=connection, sql=f"CREATE TABLE {left} AS {test_case.left_sql}")
    adapter.execute(connection=connection, sql=f"CREATE TABLE {right} AS {test_case.right_sql}")
