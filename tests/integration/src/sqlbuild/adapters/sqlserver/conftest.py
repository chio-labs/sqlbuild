...
def connection(
    ...
    adapter.execute(
        connection=conn, sql=f"CREATE SCHEMA {adapter.render_identifier(sqlserver_schema)}"
    )
    try:
        yield conn
    finally:
        ...
        adapter.execute(
            connection=conn, sql=f"DROP SCHEMA {adapter.render_identifier(sqlserver_schema)}"
        )
        adapter.close(conn)
