...
See [Testing](/docs/concepts/testing) for SQL unit tests and [Scenarios](/docs/concepts/scenarios) for local
E2E replay.
...
- **Turn review decisions into compiler checks.** Configure [Rules](/docs/concepts/rules) over compiler-owned SQL, models, dependencies, contracts, tests, and project paths.
...
[Virtual environments](/docs/concepts/virtual-environments) add version reuse and optional stale-driven
...
By default, SQLBuild runs in direct mode with state as append-only rows in your warehouse. When you want more, [virtual environments](/docs/concepts/virtual-environments) add a reversibility layer on top:
...
DuckDB runs entirely locally, so you can try SQLBuild and run full E2E tests without any warehouse credentials. Head to the [Quickstart](/docs/quickstart) to get a project running in minutes, or see [Adapters](/docs/concepts/adapters) for connection setup.
...
SQL models never depend on Python nodes - the only path from Python into SQL is a loader populating a source - so the SQL graph stays fully analyzable on its own. Nodes can also be generated programmatically with `@factory`. See [Python Nodes](/docs/concepts/python-nodes/overview).
...
<CardGroup cols={2}>
  ...
  <Card title="Scenarios" icon="flask-vial" href="/docs/concepts/scenarios">
