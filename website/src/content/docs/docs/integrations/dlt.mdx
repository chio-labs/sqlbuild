...
- **dlt inside a Python loader** - wrap any `dlt.pipeline(...)` in a SQLBuild [source loader](/docs/concepts/python-nodes/loaders) for full control when you need dlt features beyond the declarative surface.
...
When you need dlt capabilities beyond the declarative surface (custom transforms, `delete_insert`, sources not covered above), wrap a dlt pipeline in a [source loader](/docs/concepts/python-nodes/loaders). The loader calls `dlt.pipeline(...).run(...)` and returns `None`; dlt handles the writes and SQLBuild treats the source as loaded.
...
See [Loaders](/docs/concepts/python-nodes/loaders) for write strategies, the loader context API, auto-load behavior, and source deferral.
