...
When `retention_days` is not set, direct mode uses `14` and virtual-environment mode uses `30`. See [Project Configuration](/docs/concepts/project-configuration#janitor) for every janitor setting.
...
In virtual-environment mode, the janitor also cleans up expired environments, old checkpoints and unreferenced physical versions. See [Virtual environments: janitor](/docs/concepts/virtual-environments/janitor). Direct-mode archiving doesn't apply there. In both modes, relations whose names differ only by case from another relation in the same schema are reported and never dropped, and exclude patterns match regardless of case.
