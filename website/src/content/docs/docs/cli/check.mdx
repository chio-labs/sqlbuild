...
Runs Python [checks](/docs/concepts/python-nodes/checks) in isolation. Checks validate the output of tasks, assets, and loaders. For SQL relation validation, use [`sqb audit`](/docs/cli/audit) instead.
...
Selecting a non-check node is rejected; use [`sqb build`](/docs/cli/build) to run tasks and assets.
