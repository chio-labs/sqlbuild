...
Orchestrate dbt and SQLBuild together. Each subcommand runs dbt first, then SQLBuild, with selection logic across both project graphs. dbt remains responsible for dbt-owned models; SQLBuild validates and executes SQLBuild-owned models downstream. See [Using SQLBuild with dbt](/docs/concepts/dbt-compatibility/overview) for scope and selection behavior.
...
See [Selection](/docs/concepts/dbt-compatibility/selection) for full details on how selectors route work between dbt and SQLBuild.
...
See [Project Configuration](/docs/concepts/project-configuration#dbt) for all fields.
