...
- **Column lineage**: Traces which source columns flow into each output column, including transform classification. See [Column Lineage](/docs/concepts/column-lineage) for details

<Note>
`sqb compile` is authoritative for mandatory correctness, contracts, lineage, and configured Rules.
Rules findings block artifact completion. Use [`sqb rules run`](/docs/cli/rules) to focus on one exact
...
The JSON compile report includes a lineage summary for each model, not the full column graph. Use `sqb lineage <model>[.<column>]` to inspect lineage as a tree, edge list, or JSON. See [Column Lineage](/docs/concepts/column-lineage) for details on analysis modes and transform types.
