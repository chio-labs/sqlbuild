...
scope. [Virtual environments](/docs/concepts/virtual-environments) can additionally use
...
After a successful build, SQLBuild writes version identities to `_sqlbuild_fingerprints` in the target schema. These are used on subsequent runs to detect changes and skip unchanged work. See [Planning and Change Detection](/docs/concepts/planning) for details.
