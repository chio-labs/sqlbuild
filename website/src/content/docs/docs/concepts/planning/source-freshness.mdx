...
also participates in [stale-driven execution](/docs/concepts/virtual-environments/building#stale-driven-execution).

## Configuration

Source freshness is configured per source in `sources/*.yml` with a `freshness:` block. See [Sources: Source freshness](/docs/concepts/sources#source-freshness) for the full configuration reference.
...
Use [`sqb freshness`](/docs/cli/freshness) to observe source freshness on demand without triggering a build.
