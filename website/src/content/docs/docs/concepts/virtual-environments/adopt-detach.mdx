...
Detached VDE refs and state rows are cleaned up by the [janitor](/docs/concepts/virtual-environments/janitor), not by detach itself. This preserves refs for recovery if detach fails partway through.
