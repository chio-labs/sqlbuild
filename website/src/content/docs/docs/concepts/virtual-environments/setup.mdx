...
| `unsuffixed_virtual_env` | No | VDE name that uses the base schema without a suffix (for [adopt/detach](/docs/concepts/virtual-environments/adopt-detach)) |
...
This setting is also required for [adopt and detach](/docs/concepts/virtual-environments/adopt-detach) operations so that existing consumer queries continue to work after migrating to virtual mode.
...
See [Janitor](/docs/concepts/virtual-environments/janitor) for details on what gets cleaned up and how retention works.
