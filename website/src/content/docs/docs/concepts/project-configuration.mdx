...
Macro, constant, and enum visibility is not configured in either file. Declaration scopes use filesystem conventions; see [Declaration Scopes](/docs/concepts/declaration-scopes).
...
| `adapter` | Database adapter: `duckdb`, `motherduck`, `snowflake`, `bigquery`, `databricks`, `postgres`, or `sqlserver`. See [Adapters](/docs/concepts/adapters). |
...
| `defer_sources_to` | Target name to read managed source data from (see [Loaders](/docs/concepts/python-nodes/loaders#source-deferral)) |
...
This setting does not make unsupported adapter features portable. In particular, SQL Server rejects native arrays, and BigQuery rejects nested arrays. See [Collections and Rendering](/docs/concepts/constants/collections-and-rendering#project-default) for syntax, adapter output, and value-list usage constraints.
...
| `auto_load_sources` | `true` | Automatically run source loaders before building dependent models during `sqb build`. See [Loaders](/docs/concepts/python-nodes/loaders). |
...
[Compiler-integrated Rules](/docs/concepts/rules) for configuration, authoring, and suppressions.
...
See [Macros](/docs/concepts/macros) for details on variable substitution and how variables interact with macros.
...
See [janitor](/docs/cli/janitor) for the archive and delete lifecycle.
...
See [Scenarios](/docs/concepts/scenarios) for details on local type overrides and capture limits.
...
Paths can be absolute or relative to the SQLBuild project root. See [Using SQLBuild with dbt](/docs/concepts/dbt-compatibility/overview) for setup and usage details.
...
See [skills CLI reference](/docs/cli/skills) for usage details.
