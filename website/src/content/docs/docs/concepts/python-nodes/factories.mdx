...
A factory is a function that **generates** Python nodes instead of authoring them one at a time. Use `@factory` when you want to create many similar [tasks](/docs/concepts/python-nodes/tasks), [assets](/docs/concepts/python-nodes/assets), [loaders](/docs/concepts/python-nodes/loaders), or [checks](/docs/concepts/python-nodes/checks) from a list, a config, or a loop, rather than hand-writing each one.
...
Generated nodes follow the same rules as directly-authored ones, including the [SQL boundary](/docs/concepts/python-nodes/overview#the-sql-boundary): a factory-generated loader still binds to a managed source, and a factory-generated check still may not depend on SQL models.
