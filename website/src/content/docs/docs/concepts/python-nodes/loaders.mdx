...
Loaders are Python functions that load data into source tables. They replace expression sources and manual ETL scripts with code that lives inside your project, runs as part of the build, and supports incremental write strategies. Loaders are one of the four [Python node](/docs/concepts/python-nodes/overview) kinds, and the only one that writes into a SQL source.
...
Self-managed loaders must not declare a `write_strategy`. They are useful when you want to use adapter-specific SQL (e.g. `COPY INTO`, external tables), call an external ingestion tool like [dlt](/docs/integrations/dlt), or handle writes in a way that doesn't fit the dict-return pattern.
