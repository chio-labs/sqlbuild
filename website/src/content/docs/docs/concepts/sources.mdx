...
See [How Visibility Works](/docs/concepts/declaration-scopes/visibility).
...
See [Loaders](/docs/concepts/python-nodes/loaders) for the full guide on writing loader functions, write strategies, the loader context API, and auto-load behavior during builds.
...
- [dlt](/docs/integrations/dlt) - declare `dlt_sources` for `rest_api`, `sql_database`, and `filesystem` sources.
- [ingestr](/docs/integrations/ingestr) - add an `ingestr` block to a source to pull from 50+ sources.

## Source freshness

Source freshness lets SQLBuild observe whether a source's data changed between runs. This feeds into
[planning and change detection](/docs/concepts/planning): changed observations propagate to downstream
models and can alter their planned action. Virtual environments can also use the signal for
[stale-driven execution](/docs/concepts/virtual-environments/building#stale-driven-execution).
...
Use [`sqb freshness`](/docs/cli/freshness) to observe source freshness on demand without triggering a build. See [Source freshness](/docs/concepts/planning/source-freshness) for how freshness feeds into change-aware builds.
...
| `managed` | Set to `true` to bind the source to the `@loader` function of the same name (see [Loaders](/docs/concepts/python-nodes/loaders)) |
