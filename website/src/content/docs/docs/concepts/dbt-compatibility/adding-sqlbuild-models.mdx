...
- **Unit tests** can mock dbt model dependencies with `__dbt_ref__` fixture CTEs, so you can test a SQLBuild model without a warehouse connection or a compiled dbt manifest. See [Testing](/docs/concepts/testing).
- **Audits** declared inline in the `MODEL()` header block promotion when they fail, the same as in a standalone SQLBuild project. See [Audits](/docs/concepts/audits).
- **Scenarios** run the combined graph against fixture inputs for end-to-end checks. See [Scenarios](/docs/concepts/scenarios).

For the full model authoring reference (materializations, incremental strategies, contracts, and more), see [Models](/docs/concepts/models).
