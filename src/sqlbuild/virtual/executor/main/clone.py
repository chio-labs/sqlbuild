...
def run_virtual_clone(
    *,
    ...
    try:
        ...
        with OperationLifecycle(operation_kind="clone", operation_name="clone_state_connection"):
        ...
        try:
            ...
            with run_scope(clone_pipeline.destination_project.run_id):
                with OperationLifecycle(
                    operation_kind="clone", operation_name="clone_execution"
                ) as lifecycle:
                    context: CloneProjectContext = build_clone_project_context(clone_pipeline)
                    with OperationLifecycle(
                        operation_kind="clone", operation_name="clone_state_inspection"
                    ):
                    ...
                    result: VirtualCloneResult = VirtualCloneResult(
                        mode=versions.mode,
