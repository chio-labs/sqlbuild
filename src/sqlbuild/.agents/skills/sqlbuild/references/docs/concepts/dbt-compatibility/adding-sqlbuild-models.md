...
- **Unit tests** can mock dbt model dependencies with `__dbt_ref__` fixture CTEs, so you can test a SQLBuild model without a warehouse connection or a compiled dbt manifest. See [Testing](../testing.md).
- **Audits** declared inline in the `MODEL()` header block promotion when they fail, the same as in a standalone SQLBuild project. See [Audits](../audits.md).
- **Scenarios** run the combined graph against fixture inputs for end-to-end checks. See [Scenarios](../scenarios.md).

For the full model authoring reference (materializations, incremental strategies, contracts, and more), see [Models](../models.md).
