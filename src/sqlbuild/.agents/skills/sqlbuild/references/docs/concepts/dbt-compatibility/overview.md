...
| ---- | --------- | ----- |
| `--select` / `--exclude` | dbt | dbt resolves selection (see [Selection](selection.md)). |
...
- [Selection](selection.md) - how `--select` and `--exclude` route work across both graphs.
- [Adding SQLBuild models](adding-sqlbuild-models.md) - optionally write SQLBuild models, tests, audits, and scenarios downstream of dbt.
