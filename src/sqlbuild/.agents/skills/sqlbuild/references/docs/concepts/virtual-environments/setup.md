...
Online: https://docs.sqlbuild.com/concepts/virtual-environments/setup
...
| `unsuffixed_virtual_env` | No | VDE name that uses the base schema without a suffix (for [adopt/detach](adopt-detach.md)) |
...
This setting is also required for [adopt and detach](adopt-detach.md) operations so that existing consumer queries continue to work after migrating to virtual mode.
...
See [Janitor](janitor.md) for details on what gets cleaned up and how retention works.
