...
Assets are Python nodes that produce or observe an external artifact - an exported file, a published dataset, a dashboard refresh, an ML model. They behave like [tasks](tasks.md) but add dataset-like metadata (columns, column lineage) and a materialization flag. See [Python Nodes](overview.md) for the shared model and the SQL boundary rules.
...
To read a SQL model or source, declare a typed reference and resolve it at runtime - see [SQL references](sql-references.md):
...
See [Tasks](tasks.md#retries) for the full retry policy fields.
