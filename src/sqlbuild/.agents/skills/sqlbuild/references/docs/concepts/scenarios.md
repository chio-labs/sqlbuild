...
[SQL unit tests](testing.md) compile to a single comparison query. They are fast, inline, and work well for checking individual model transformations. But when you need to test across many models with related entities - customers that have orders, orders that have payments, refunds that reference original orders - a single query can become unwieldy or hit SQL size limits.
...
[How Visibility Works](declaration-scopes/visibility.md#tests-and-expected-output).
...
See the [CLI reference](../cli/scenario.md) for full command documentation.
