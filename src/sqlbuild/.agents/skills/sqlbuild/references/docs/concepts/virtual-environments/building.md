...
Online: https://docs.sqlbuild.com/concepts/virtual-environments/building
...
See [Planning and change detection](../planning.md) for the shared identity and cascade model.
...
Source freshness observations and Python node identities are also persisted per virtual environment and included in version hash computation. When a source's observed data version changes, its downstream models become stale. See [Sources: Source freshness](../sources.md#source-freshness) for configuration.
