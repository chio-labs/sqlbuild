...
Text output is deterministic, bounded, and requires no interactive pager. In a color terminal it
uses the same color conventions as other commands; redirected output and `NO_COLOR` produce plain
text.
...
| `--verbose`, `-v` | Show each declaration's scope details and column, and keep empty optional sections. |
...
By default, each text row shows a usage marker, the declaration's qualified identity, and its
`file:line` definition. A one-line legend explains the markers. Empty optional sections
(**Relationship grants**, **Nearby unavailable**, **Private retained**, **Relationship retained**,
and **Diagnostics**) are omitted; the core sections are always shown, even when empty.

`--verbose` restores the full rows: kind, visibility, reason for appearing, declaration-role root,
safe type or signature metadata where useful, and the definition column. It also shows empty
optional sections. `--explain` always shows the explained declaration in full, plus its owning
path, narrowest required placement, current consumers, dependencies, and the consumers affected by
a placement mismatch. JSON includes the complete structured declaration report regardless of
`--verbose`.
...
Used (1)
  └─ ● macro:formatted_order_total  models/commerce/macros/orders.py:5
...
With `--verbose`, the same `Used` section shows why each declaration is visible:

```console
$ sqb scope model:orders --verbose
Used (1)
...
macro imports, see [Declarations and Scopes](../concepts/declaration-scopes.md).
