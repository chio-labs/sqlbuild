...
keys retain inherited-prefix behavior, so `market/partner_feed/staging` applies to every model
...
`models/market/staging/` and `models/market/partner_feed/staging/`. Wildcards must be complete
