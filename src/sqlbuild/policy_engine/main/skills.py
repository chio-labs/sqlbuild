"""Generate deterministic agent guidance from the resolved project policy."""
...
from sqlbuild.policy_engine._helpers.engine.native import native_skill_freshness
from sqlbuild.policy_engine._helpers.guidance.skill_installation import install_rendered_skills
...
def install_skills(*, config: PolicyConfig, project_dir: Path, check: bool) -> bool:
    ...
    targets: tuple[Path, ...] = tuple(project_dir / value for value in POLICY_SKILL_PATHS)
    if check:
        return all(
    ...
    lock_dir: Path = Path(gettempdir()) / "sqlbuild-policy-skill-locks"
