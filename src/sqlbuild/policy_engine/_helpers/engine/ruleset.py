"""Single policy catalogue and active-ruleset resolution."""
...
from sqlbuild.policy_engine._helpers.engine.hermeticity import verify_custom_rules
from sqlbuild.policy_engine._helpers.engine.native import evaluate_native
from sqlbuild.policy_engine.models import PolicyConfig, PolicyResult, PolicyRule, ResolvedRuleset


def evaluate_project(
    *, project: CompiledProject, config: PolicyConfig, project_dir: Path, dialect: str = "generic"
) -> PolicyResult:
    """Evaluate selected rules through one native project boundary."""

    catalogue: tuple[PolicyRule, ...] = build_catalogue(config=config, project_dir=project_dir)
...
def resolve_ruleset(*, config: PolicyConfig, project_dir: Path) -> ResolvedRuleset:
    """Resolve one immutable ruleset for evaluation, inspection, caching, and skills."""

    catalogue: tuple[PolicyRule, ...] = build_catalogue(config=config, project_dir=project_dir)
    rules: tuple[PolicyRule, ...] = select_rules(
        catalogue=catalogue, config=config, project_dir=project_dir
    )
