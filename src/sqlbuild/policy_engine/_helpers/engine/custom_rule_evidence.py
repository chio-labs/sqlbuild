...
_PUBLIC_HARNESS_MODULE: str = "sqlbuild.policy"
_INIT_MODULE_NAME: str = "__init__"
_POLICY_DIRECTORY_NAME: str = "policy"
...
def custom_rule_test_evidence(
    *, rule: PolicyRule, project_dir: Path
...
def custom_rule_implementation_fingerprint(*, rule: PolicyRule, project_dir: Path) -> str:
    """Fingerprint a custom rule and repository-owned Python files in its policy tree."""
...
def _source_root(*, source_path: Path, project_dir: Path) -> Path:
    ...
    for parent in (source_path.parent, *source_path.parents):
        if parent == root:
            break
        if parent.name == _POLICY_DIRECTORY_NAME:
            return parent
