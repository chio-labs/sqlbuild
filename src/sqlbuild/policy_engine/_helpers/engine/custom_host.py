"""Bounded subprocess host for repository-defined Policy rules."""
...
from sqlbuild.policy_engine._helpers.engine.custom_evaluation import evaluate_custom_rules
...
def main() -> int:
    ...
    try:
        ...
        if not isinstance(payload, dict):
            raise PolicyError("custom host payload must be an object")
        ...
        with contextlib.redirect_stdout(messages):
            catalogue: tuple[PolicyRule, ...] = build_catalogue(
                config=config, project_dir=project_dir
            )
            by_code: dict[str, PolicyRule] = {rule.code: rule for rule in catalogue}
            selected: tuple[PolicyRule, ...] = tuple(by_code[code] for code in selected_codes)
            faults: list[PolicyFault] = evaluate_custom_rules(
                project=project,
                config=config,
...
def _decode_inputs(payload: dict[str, Any]) -> tuple[CompiledProject, PolicyConfig]:
    ...
    if (
        ...
        or not isinstance(decoded[1], PolicyConfig)
    ):
        raise PolicyError("custom host project payload has invalid types")
    return decoded


def _fault_payload(fault: PolicyFault) -> dict[str, object]:
