"""Typed Python adapter for the private native policy engine."""
...
def evaluate_native(
    *,
    ...
    if not isinstance(response, dict):
        raise PolicyError("native policy engine returned an invalid response")
    payload: dict[str, Any] = response
    if payload.get("version") != POLICY_NATIVE_API_VERSION:
        raise PolicyError("native policy engine returned an unsupported response version")
    raw_faults: object = payload.get("faults")
    if not isinstance(raw_faults, list):
        raise PolicyError("native policy engine returned invalid faults")
    return PolicyResult(
...
def load_native_config(project_dir: Path) -> dict[str, object]:
    """Load strict policy TOML through the native configuration owner."""
    ...
    if not isinstance(payload, dict):
        raise PolicyError("native policy engine returned invalid configuration")
...
def native_catalogue() -> tuple[dict[str, object], ...]:
    ...
    if not isinstance(payload, dict) or not isinstance(payload.get("rules"), list):
        raise PolicyError("native policy engine returned an invalid catalogue")
    rules: list[object] = payload["rules"]
    if any(not isinstance(value, dict) for value in rules):
        raise PolicyError("native policy engine returned invalid rule metadata")
...
def native_selected_codes(
    *, config: PolicyConfig, catalogue: tuple[PolicyRule, ...], project_dir: Path
) -> tuple[str, ...]:
    ...
    if not isinstance(payload, list) or any(not isinstance(code, str) for code in payload):
        raise PolicyError("native policy engine returned invalid selected rule codes")
...
def _config_payload(config: PolicyConfig) -> dict[str, object]:
...
def _custom_rule_payload(*, rule: PolicyRule, project_dir: Path) -> dict[str, object]:
...
def _custom_rule_source_payload(*, rule: PolicyRule, project_dir: Path) -> dict[str, object]:
    ...
    return {
        ...
        "source_column": 1,
        "owner": f"{source or 'policy'}:{check_owner}",
    }
...
def _custom_host_payload(
    *,
    ...
    return {
        ...
        "arguments": ["-m", "sqlbuild.policy_engine._helpers.engine.custom_host"],
...
def _decode_fault(value: object) -> PolicyFault:
    if not isinstance(value, dict):
        raise PolicyError("native policy engine returned an invalid fault")
