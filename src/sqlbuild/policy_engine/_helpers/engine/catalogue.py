"""Built-in and custom policy rule catalogue construction."""
...
from sqlbuild.policy_engine._helpers.engine.native import native_catalogue, native_selected_codes
from sqlbuild.policy_engine.constants import POLICY_DECORATOR_TOKEN
...
def build_catalogue(*, config: PolicyConfig, project_dir: Path) -> tuple[PolicyRule, ...]:
    ...
    rules: tuple[PolicyRule, ...] = (*_builtins(), *custom)
    ...
    if duplicates:
        raise PolicyError(f"duplicate policy rule codes: {', '.join(duplicates)}")
    for rule in rules:
...
def _builtins() -> tuple[PolicyRule, ...]:
    return tuple(
        PolicyRule(
            code=str(item["code"]),
...
def _native_builtin(*, model: object, ctx: object) -> list[PolicyFault]:
    del model, ctx
    raise PolicyError("built-in policy rules execute only through the native engine")


def select_rules(
    *, catalogue: tuple[PolicyRule, ...], config: PolicyConfig, project_dir: Path
) -> tuple[PolicyRule, ...]:
    """Resolve the active catalogue through the native Fensu policy owner."""

    by_code: dict[str, PolicyRule] = {rule.code: rule for rule in catalogue}
    ...
    try:
        ...
        raise PolicyError(f"native project policy returned unknown rule {error.args[0]}") from error


def _load_modules(*, names: tuple[str, ...], project_dir: Path) -> tuple[PolicyRule, ...]:
    ...
    try:
        for name in names:
            try:
                ...
                raise PolicyError(f"could not import policy rule module {name}: {error}") from error
            ...
            if source is None or not Path(source).resolve().is_relative_to(project_dir.resolve()):
                raise PolicyError(f"policy rule module {name} must be repository-owned")
...
def _load_paths(*, paths: tuple[str, ...], project_dir: Path) -> tuple[PolicyRule, ...]:
    result: list[PolicyRule] = []
    existing_modules: frozenset[str] = frozenset(sys.modules)
    repository_path: str = str(project_dir.resolve())
    sys.path.insert(0, repository_path)
    try:
        for configured in paths:
            ...
            if not path.is_relative_to(project_dir.resolve()):
                raise PolicyError(f"policy rule path escapes the project: {configured}")
            ...
            for file_path in files:
                if not file_path.is_file():
                    raise PolicyError(f"policy rule path does not exist: {file_path}")
                if configured_directory and POLICY_DECORATOR_TOKEN not in file_path.read_text(
                    encoding="utf-8"
                ):
                    continue
                digest: str = hashlib.sha256(str(file_path).encode()).hexdigest()[:16]
                name: str = f"sqlbuild._loaded_policy.{digest}"
                spec: ModuleSpec | None = importlib.util.spec_from_file_location(name, file_path)
                if spec is None or spec.loader is None:
                    raise PolicyError(f"could not load policy rule file {file_path}")
                ...
                try:
                    ...
                    raise PolicyError(
                        f"could not import policy rule file {file_path}: {error}"
                    ) from error
                finally:
...
def _module_rules(
    *, module: ModuleType, source: Path, require_rules: bool = True
    ...
    if not rules and require_rules:
        raise PolicyError(f"policy rule source exposes no @policy rules: {source}")
...
def _validate_options(*, rule: PolicyRule, configured: Mapping[str, object]) -> None:
    ...
    if unknown:
        raise PolicyError(f"rule {rule.code} has unknown options: {', '.join(sorted(unknown))}")
    for name, value in configured.items():
        ...
        if not valid:
            raise PolicyError(f"rule {rule.code} option {name} has the wrong type")
        if option.choices and value not in option.choices:
            raise PolicyError(f"rule {rule.code} option {name} is not an allowed choice")
        if isinstance(value, tuple):
            if option.minimum_items is not None and len(value) < option.minimum_items:
                raise PolicyError(f"rule {rule.code} option {name} has too few items")
        ...
        if isinstance(value, int) and not isinstance(value, bool):
            if option.minimum is not None and value < option.minimum:
                raise PolicyError(f"rule {rule.code} option {name} is below its minimum")
            if option.maximum is not None and value > option.maximum:
                raise PolicyError(f"rule {rule.code} option {name} exceeds its maximum")


def _validate_option_declaration(*, rule: PolicyRule, option: RuleOption[object]) -> None:
    if not option.name.strip() or not option.description.strip():
        raise PolicyError(f"rule {rule.code} declares an incomplete option")
    ...
    if not valid_default:
        raise PolicyError(f"rule {rule.code} option {option.name} has a default of the wrong type")
    if option.choices and default not in option.choices:
        raise PolicyError(f"rule {rule.code} option {option.name} default is not an allowed choice")
    if isinstance(default, tuple):
        if option.minimum_items is not None and len(default) < option.minimum_items:
            raise PolicyError(f"rule {rule.code} option {option.name} default has too few items")
    ...
    if isinstance(default, int) and not isinstance(default, bool):
        if option.minimum is not None and default < option.minimum:
            raise PolicyError(f"rule {rule.code} option {option.name} default is below its minimum")
        if option.maximum is not None and default > option.maximum:
            raise PolicyError(f"rule {rule.code} option {option.name} default exceeds its maximum")
