...
from sqlbuild.policy_engine.constants import REPLACEABLE_SKILL_STATES
    ...
    *, content: str, input_fingerprint: str, targets: tuple[Path, ...]
    ...
    for path, state in zip(targets, freshness, strict=True):
        if state not in REPLACEABLE_SKILL_STATES:
            raise PolicyError(f"refusing to overwrite {state} policy skill: {path}")
    ...
    *, staged: tuple[tuple[Path, Path | None, Path], ...], installed_bytes: bytes
    ...
    try:
        for temporary, backup, target in staged:
            ...
            if current != expected:
                raise PolicyError(
                    f"refusing to overwrite policy skill changed during installation: {target}"
                )
