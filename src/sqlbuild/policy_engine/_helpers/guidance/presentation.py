"""Deterministic policy result and rule rendering."""
...
_MINIMUM_POLICY_CODES: frozenset[str] = frozenset({"SQBPT201", "SQBPT202"})


def format_result_text(*, result: PolicyResult) -> str:
    if not result.faults:
        return (
            f"Project Policy passed: {result.evaluated_models} models evaluated, 0 faults "
    ...
    blocks.append(f"Found {len(result.faults)} Project Policy faults")
    return "\n".join(blocks)


def format_result_json(*, result: PolicyResult) -> str:
...
def format_rule_text(*, rule: PolicyRule, config: PolicyConfig | None = None) -> str:
    lines: list[str] = [
    ...
    if config is not None and rule.code in _MINIMUM_POLICY_CODES:
        lines.extend(("", "Effective thresholds:", *format_threshold_lines(config=config)))
    if config is not None and rule.code.startswith("SQBPT"):
        lines.extend(
            (
                "",
