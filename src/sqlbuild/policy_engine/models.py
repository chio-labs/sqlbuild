"""Structured policy configuration, rule, and evaluation models."""
...
from sqlbuild.policy_engine.types import PolicyCheck, RuleOptionValue


@dataclass(frozen=True)
class RuleOption[T]:
    """One typed configuration option declared by a policy rule."""
...
@dataclass(frozen=True)
class PolicyRule:
    """Immutable metadata and implementation for one policy rule."""
...
@dataclass(frozen=True)
class PolicyFault:
    """One deterministic project policy violation."""
...
class PolicyConfig:
    ...
    cache: PolicyCacheConfig = field(default_factory=PolicyCacheConfig)


@dataclass(frozen=True)
class PolicyResult:
    faults: tuple[PolicyFault, ...]
