...
def format_debug_text(result: DebugResult, *, use_color: bool) -> str:
    ...
    _append_section(lines, section_name="Runtime", section_lines=result.runtime, style=style)
    _append_section(
        lines, section_name="Configuration", section_lines=result.configuration, style=style
    )
    _append_section(lines, section_name="Providers", section_lines=result.providers, style=style)
    _append_section(lines, section_name="Connection", section_lines=result.connection, style=style)
    return "\n".join(lines) + "\n"
...
def _append_section(
    ...
    for line in section_lines:
        ...
        if line.status is not None:
            ...
            status: str = _format_status(line.status, message=status_message, style=style)
...
def _format_status(status: DebugCheckStatus, *, message: str, style: CliStyle) -> str:
    ...
    return style.status(status_text, text=rendered)
