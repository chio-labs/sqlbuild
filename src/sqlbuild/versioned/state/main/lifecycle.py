...
from sqlbuild.versioned.state.helpers.config import resolve_state_backend_config
from sqlbuild.versioned.state.models import StateBackendConfig, StateSchemaValidationResult
from sqlbuild.versioned.state.types import StateBackendName, StateCommand
...
def run_state_lifecycle(
    *,
    ...
    """Run a versioned state lifecycle command."""
    ...
    try:
        if command == StateCommand.INIT:
            ...
            print(f"Initialized versioned state schema '{config.schema}'.")
            return 0
        if command == StateCommand.MIGRATE:
            ...
            print(
                f"Migrated versioned state schema '{config.schema}' "
        ...
        if command == StateCommand.ROLLBACK:
            ...
            print(f"Rolled back versioned state schema '{config.schema}' to '{used_backup_id}'.")
            return 0
        if command == StateCommand.RESET:
            ...
            print(f"Reset versioned state schema '{config.schema}'.")
            return 0
