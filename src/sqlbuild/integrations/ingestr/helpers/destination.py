...
def _duckdb_uri(config: dict[str, object]) -> str:
    database: str = _required_string(config, "database", adapter_name=BuiltinAdapter.DUCKDB.value)
    return f"duckdb:///{database}"


def _motherduck_uri(config: dict[str, object]) -> str:
    database: str = _optional_string(config, "database") or ""
    if database.startswith("md:"):
        database = database.removeprefix("md:")
    token: str | None = _optional_string(config, "token")
...
def _postgres_uri(config: dict[str, object]) -> str:
    ...
    sslmode: str | None = _optional_string(config, "sslmode")
...
def _bigquery_uri(config: dict[str, object]) -> str:
    project: str = _optional_string(config, "project") or _required_string(
        config, "database", adapter_name=BuiltinAdapter.BIGQUERY.value
    )
    params: dict[str, str] = {}
    for key in ("location", "credentials_path", "credentials_base64"):
        value: str | None = _optional_string(config, key)
...
def _snowflake_uri(config: dict[str, object]) -> str:
    ...
    user: str = _required_string(config, "user", adapter_name=BuiltinAdapter.SNOWFLAKE.value)
    password: str = _optional_string(config, "password") or ""
    database: str = _required_string(
        config, "database", adapter_name=BuiltinAdapter.SNOWFLAKE.value
    )
    schema: str = _required_string(config, "schema", adapter_name=BuiltinAdapter.SNOWFLAKE.value)
    params: dict[str, str] = {}
    for key in (
        "warehouse",
        "role",
...
def _databricks_uri(config: dict[str, object]) -> str:
    ...
    token: str = _required_string(config, "token", adapter_name=BuiltinAdapter.DATABRICKS.value)
    params: dict[str, str] = {
...
def _sqlserver_uri(config: dict[str, object]) -> str:
    host: str = (
        _optional_string(config, "host") or _optional_string(config, "server") or "localhost"
    )
    port: object = config.get("port", 1433)
    user: str = _optional_string(config, "user") or _optional_string(config, "username") or "sa"
    password: str = _optional_string(config, "password") or ""
    database: str = (
        _optional_string(config, "database") or _optional_string(config, "dbname") or "master"
    )
    params: dict[str, str] = {}
