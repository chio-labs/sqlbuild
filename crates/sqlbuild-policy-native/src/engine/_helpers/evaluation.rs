...
pub(crate) fn evaluate_json(request_json: &str) -> Result<String, String> {
    ...
    let custom_project_wide_selected = selected.iter().any(|rule| rule.custom && rule.project_wide);
    ...
    let project_fingerprint = if project_aware {
        Some(if custom_project_wide_selected {
            ...
            fingerprint_request_facts(b"compiler-owned-policy-facts-v1", &request)?
        })
    } else {
        None
    };
...
fn evaluate_custom_rules_cached(
    ...
    let Some(cache) = cache else {
        return evaluate_custom_rules(request, selected, None);
    };
    let local: BTreeMap<String, &RuleMetadata> = selected
        .iter()
        .filter(|(_, rule)| rule.custom && !rule.project_wide)
        .map(|(code, rule)| (code.clone(), *rule))
        .collect();
    let project_wide: BTreeMap<String, &RuleMetadata> = selected
        .iter()
        .filter(|(_, rule)| rule.custom && rule.project_wide)
        .map(|(code, rule)| (code.clone(), *rule))
        .collect();
    ...
    if !project_wide.is_empty() {
        ...
        if let Some(cached) = cache.get("__custom_policy_project__", &identity)? {
            ...
            let evaluated = evaluate_custom_rules(request, &project_wide, None)?;
            cache.put("__custom_policy_project__", &identity, &evaluated)?;
            faults.extend(evaluated);
        }
    }
    if local.is_empty() {
        return Ok(faults);
    ...
    for model in &request.models {
        let identity = custom_local_identity(model, ruleset_fingerprint)?;
        let cache_path = custom_model_cache_path(&model.relative_path);
    ...
    let paths: BTreeSet<String> = misses.keys().cloned().collect();
    let evaluated = evaluate_custom_rules(request, &local, Some(&paths))?;
    if let Some(fault) = evaluated.iter().find(|fault| !paths.contains(&fault.path)) {
        return Err(format!(
            "model-local custom policy returned a finding for unrelated path {}; declare the rule project-wide",
            fault.path
        ));
    }
    for (path, identity) in misses {
        let model_faults: Vec<Fault> = evaluated
            ...
            .cloned()
            .collect();
        cache.put(&custom_model_cache_path(&path), &identity, &model_faults)?;
        faults.extend(model_faults);
    }
    Ok(faults)
}

fn custom_model_cache_path(model_path: &str) -> String {
    format!("__custom_models__/{model_path}")
}

fn custom_project_identity(ruleset: &str, project: Option<&str>) -> String {
    let mut digest = Sha256::new();
    digest.update(b"custom-policy-project-v2");
    digest.update(ruleset.as_bytes());
    digest.update(project.unwrap_or("no-project-fingerprint").as_bytes());
    format!("{:x}", digest.finalize())
}

fn custom_local_identity(model: &crate::models::Model, ruleset: &str) -> Result<String, String> {
    let model_identity = model_cache_identity(model, ruleset, None)?;
    let mut digest = Sha256::new();
    digest.update(b"custom-policy-model-v1");
...
fn model_cache_identity(
    ...
    let request = AnalysisBatchRequest {
        ...
        identity: RuntimeIdentity {
            producer: "sqlbuild-policy".to_owned(),
            fact_schema: "sqlbuild-policy-model-v1".to_owned(),
...
fn fingerprint_project(project_dir: &Path, request: &EvaluateRequest) -> Result<String, String> {
    ...
    for path in files {
        ...
        digest.update(std::fs::read(&path).map_err(|error| {
            format!(
                "could not fingerprint policy project file {}: {error}",
                path.display()
            )
        })?);
    }
...
fn collect_policy_files(root: &Path, directory: &Path) -> Result<Vec<PathBuf>, String> {
    ...
    for entry in std::fs::read_dir(directory)
        .map_err(|error| format!("could not read policy project directory: {error}"))?
    {
        let entry =
            entry.map_err(|error| format!("could not read policy project entry: {error}"))?;
        let path = entry.path();
        if path.is_dir() {
            if path
                .file_name()
...
fn validate_suppression_codes(
    ...
    if !unknown.is_empty() {
        return Err(format!(
            "policy exceptions target unknown rules: {}",
    ...
    for ignore in &request.config.rule_ignores {
        for selector in &ignore.rules {
            if !catalogue.iter().any(|rule| rule.code.starts_with(selector)) {
                return Err(format!(
                    "policy rule-ignore selector matches no rules: {selector}"
                ));
            }
        }
    }
...
fn validate_custom_faults(
    ...
    for fault in faults {
        if !selected.get(&fault.code).is_some_and(|rule| rule.custom) {
            return Err(format!(
                "custom policy host returned a fault for unselected rule {}",
                fault.code
            ));
        }
    }
...
fn evaluate_custom_rules(
    ...
    let Some(spec) = &request.custom_host else {
        return if custom_selected {
            Err("selected custom policy rules require a custom host".to_owned())
        } else {
            Ok(Vec::new())
        };
    };
    ...
    let payload_object = payload
        .as_object_mut()
        .ok_or_else(|| "custom policy host payload must be an object".to_owned())?;
    ...
    let faults = response.payload.ok_or_else(|| {
        "custom policy host returned no fault payload after successful validation".to_owned()
    })?;
...
fn validate_exception_paths(request: &EvaluateRequest) -> Result<(), String> {
    ...
    for entry in &request.config.rule_exceptions {
        if !root.join(&entry.path).is_file() {
            return Err(format!(
                "policy exception path does not exist: {}",
                entry.path
            ));
        }
    }
...
fn suppress_faults(
    ...
    let scoped_ignores = request
        ...
        .collect::<Vec<_>>();
    let grammar = ProductRuleCodeGrammar::new("SQBP", "XSQBP")?;
...
fn fault_to_finding(fault: Fault) -> Result<Finding, String> {
    Ok(Finding {
        code: fault.code,
        path: fault.path,
        line: Some(u32::try_from(fault.line).map_err(|_| "policy fault line exceeds u32")?),
        column: Some(u32::try_from(fault.column).map_err(|_| "policy fault column exceeds u32")?),
        symbol: None,
...
fn lifecycle_error(error: LifecycleError) -> String {
    match error {
        LifecycleError::StaleSuppression { code, path, .. } => {
            format!("stale policy exception suppresses no fault: {code} at {path}")
        }
        other => format!("invalid policy lifecycle: {other}"),
    }
}
