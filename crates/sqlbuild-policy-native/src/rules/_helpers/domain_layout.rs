...
fn evaluate_models(evaluation: &ProjectEvaluationRequest<'_>) -> Vec<Fault> {
    ...
    for model in &evaluation.request.models {
        let resolved = match resolve_model_path(model, &levels, &domain_roots) {
            ...
            ModelPathResolution::Missing => {
                if let Some(rule) = evaluation.selected.get("SQBPR201") {
            ...
            ModelPathResolution::Ambiguous(candidates) => {
                if let Some(rule) = evaluation.selected.get("SQBPR201") {
                    faults.push(path_fault(
                        ...
                        "Set policy.layout.domain_roots to the intended normalized roots.".into(),
                    ));
                }
                continue;
            }
        };
        if let Some(rule) = evaluation.selected.get("SQBPR203")
            && resolved.owners.len() as u32 > max_depth
        {
            faults.push(path_fault(
                ...
                format!(
                    "Flatten this path beneath {}/{}, promote part of it into the domain root, or set policy.thresholds.max_subdomain_depth to at least {}.",
                    resolved.domain,
                    resolved.level,
                    resolved.owners.len()
                ),
    ...
    if min_prefixes > 0
        && let Some(rule) = evaluation.selected.get("SQBPR204")
    {
...
fn inspect_owner(policy: &OwnerInspection<'_>, node: &OwnerNode, owners: &[String]) -> Vec<Fault> {
    let mut faults: Vec<Fault> = Vec::new();
    if let Some(rule) = policy.evaluation.selected.get("SQBPR202")
    ...
    if policy.min_prefixes > 0
        && let Some(rule) = policy.evaluation.selected.get("SQBPR204")
    {
...
fn inspect_container(policy: &ContainerInspection<'_>, entries: &[ContainerEntry]) -> Vec<Fault> {
    ...
    let grouped: Vec<&ContainerEntry> = entries
        ...
        .filter(|entry| !entry.buckets.is_empty())
        .collect();
    if let Some(rule) = policy.evaluation.selected.get("SQBPD302")
        && !direct.is_empty()
        && !grouped.is_empty()
    {
        faults.push(path_fault(
    ...
    if let Some(rule) = policy.evaluation.selected.get("SQBPD303") {
            ...
            .filter(|entry| entry.buckets.len() as u32 > policy.max_depth)
        {
            faults.push(path_fault(
                ...
                format!(
                    "Flatten this bucket path or set policy.thresholds.max_role_container_depth to at least {}.",
                    entry.buckets.len()
                ),
            ));
        }
    }
    if let Some(rule) = policy.evaluation.selected.get("SQBPD305") {
    ...
    if let Some(rule) = policy.evaluation.selected.get("SQBPD304") {
    ...
    if policy.min_prefixes > 0
        && let Some(rule) = policy.evaluation.selected.get("SQBPD306")
    {
