...
pub(crate) fn evaluate_project(
    ...
    if let Some(rule) = evaluation.selected.get("SQBPT101") {
        faults.extend(canonical_roots(rule, &tests, &scenarios));
    }
    if let Some(rule) = evaluation.selected.get("SQBPT102") {
        faults.extend(filenames(rule, &evaluation, &tests, &scenarios));
    }
    if let Some(rule) = evaluation.selected.get("SQBPT103") {
        faults.extend(mirroring(rule, &evaluation, &tests));
    }
    if let Some(rule) = evaluation.selected.get("SQBPT104") {
        faults.extend(structured_names(rule, &evaluation, &tests));
    }
    if let Some(rule) = evaluation.selected.get("SQBPT105") {
