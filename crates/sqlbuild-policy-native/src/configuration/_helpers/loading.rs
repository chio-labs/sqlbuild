...
pub(crate) fn load(project_dir: &Path) -> Result<PolicyConfig, String> {
    let path = project_dir.join("sqlbuild_project.toml");
    let source = std::fs::read_to_string(&path).map_err(|error| {
        format!(
            "could not load policy config from {}: {error}",
            path.display()
        )
    })?;
    ...
    if root.contains_key("kata") {
        return Err("unsupported legacy [kata] configuration; use [policy]".to_owned());
    }
    let config = match root.get("policy") {
        None => PolicyConfig::default(),
        Some(value) => {
            validate_raw(value)?;
            value
                .clone()
                .try_into::<PolicyConfig>()
                .map_err(|error| format!("invalid policy config: {error}"))?
        }
    };
    validate(&config)?;
    Ok(config)
...
fn validate_raw(value: &toml::Value) -> Result<(), String> {
    let table = value
        .as_table()
        .ok_or_else(|| "policy must be a table".to_owned())?;
    let known = [
        "select",
        "ignore",
        "thresholds",
        ...
        for entry in entries {
            ...
            for field in fields {
                if !entry_table.contains_key(*field) {
                    return Err(format!("policy.{key}.{field} must be a non-empty value"));
                }
            }
        }
    }
...
pub(crate) fn validate(config: &PolicyConfig) -> Result<(), String> {
    ...
    if config.layout.levels.is_empty() {
        return Err(
            "policy.layout.levels must contain at least one normalized relative path".into(),
        );
    }
    let mut levels: std::collections::BTreeSet<&String> = std::collections::BTreeSet::new();
    for level in &config.layout.levels {
        validate_relative_path(level, "policy.layout.levels")?;
        if !levels.insert(level) {
            return Err(format!("duplicate policy layout level: {level}"));
        }
    }
    validate_non_overlapping_paths(&config.layout.levels, "policy.layout.levels")?;
    let mut domain_roots: std::collections::BTreeSet<&String> = std::collections::BTreeSet::new();
    for root in &config.layout.domain_roots {
        validate_relative_path(root, "policy.layout.domain_roots")?;
        if !domain_roots.insert(root) {
            return Err(format!("duplicate policy layout domain root: {root}"));
        }
    }
    validate_non_overlapping_paths(&config.layout.domain_roots, "policy.layout.domain_roots")?;
    ...
    {
        return Err(
            "policy.sql_tests.pipeline_directory must be a normalized path relative to tests/unit"
                .into(),
        );
    }
    ...
    {
        return Err(format!("unknown policy thresholds: {name}"));
    }
    for entry in &config.threshold_overrides {
        if entry.paths.is_empty() || entry.thresholds.is_empty() || entry.reason.trim().is_empty() {
            return Err("policy.threshold_overrides requires paths, thresholds, and reason".into());
        }
        ...
        {
            return Err(format!("unknown policy threshold override: {name}"));
        }
        for pattern in &entry.paths {
            if pattern.trim().is_empty() {
                return Err("policy threshold override paths must be non-empty globs".into());
            }
            globset::Glob::new(pattern).map_err(|error| {
                format!("invalid policy threshold override path {pattern}: {error}")
            })?;
        }
    }
    ...
    for selector in config.select.iter().chain(&config.ignore) {
        if !grammar.rule_selector_is_valid(selector) {
            return Err(format!("malformed policy rule selector: {selector}"));
        }
    }
    for code in config.rule_options.keys() {
        if !grammar.rule_code_is_exact(code) {
            return Err(format!("invalid policy rule code: {code}"));
        }
    }
    for exception in &config.rule_exceptions {
        ...
        {
            return Err("policy.rule_exceptions fields must be non-empty strings".into());
        }
        if !grammar.rule_code_is_exact(&exception.rule) {
            return Err(format!("invalid policy rule code: {}", exception.rule));
        }
    }
    for ignore in &config.rule_ignores {
        if ignore.rules.is_empty() || ignore.paths.is_empty() || ignore.reason.trim().is_empty() {
            return Err("policy.rule_ignores requires rules, paths, and reason".into());
        }
    ...
    for allow in &config.select_star_allow {
        if allow.paths.is_empty() || allow.reason.trim().is_empty() {
            return Err("policy.select_star_allow requires paths and reason".into());
        }
    }
    Ok(())
}
