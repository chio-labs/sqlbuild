...
fn given_additional_rule_cases_when_linting_then_findings_and_fixes_match() -> Result<(), String> {
    let test_cases = [
        ...
        test_types::AdditionalLintRuleTestCase {
            description: "qualified column self alias is redundant",
            sql: "SELECT items.value AS value FROM items",
            rule: "SQBRSQL016",
            expected_anchor: Some(" AS value"),
            expected_replacement: None,
        },
        test_types::AdditionalLintRuleTestCase {
            description: "qualified column self alias after an expression is redundant",
            sql: "SELECT item_id + 1 AS next_item_id, items.value AS value FROM items",
            rule: "SQBRSQL016",
            expected_anchor: Some(" AS value"),
            expected_replacement: None,
        },
        test_types::AdditionalLintRuleTestCase {
            description: "top-modified column self alias is redundant",
            sql: "SELECT TOP 5 value AS value FROM items",
            rule: "SQBRSQL016",
            expected_anchor: Some(" AS value"),
            expected_replacement: None,
        },
        test_types::AdditionalLintRuleTestCase {
            description: "terminal operand name does not make a calculation self-aliased",
            sql: "SELECT price + value AS value FROM items",
            rule: "SQBRSQL016",
            expected_anchor: None,
            expected_replacement: None,
        },
        test_types::AdditionalLintRuleTestCase {
            description: "cast type syntax is not an output self alias",
            sql: "SELECT CAST(order_id::INTEGER AS INTEGER) AS order_id FROM orders",
            rule: "SQBRSQL016",
            expected_anchor: None,
            expected_replacement: None,
        },
        test_types::AdditionalLintRuleTestCase {
            description: "relation self alias is not an output self alias",
            sql: "SELECT orders.order_id FROM orders AS orders",
            rule: "SQBRSQL016",
            expected_anchor: None,
            expected_replacement: None,
        },
        test_types::AdditionalLintRuleTestCase {
            description: "semi-structured path alias is not redundant",
            sql: "SELECT event.payload:data:order_id AS order_id FROM events AS event",
            rule: "SQBRSQL016",
            expected_anchor: None,
            expected_replacement: None,
        },
        ...
        test_types::AdditionalLintRuleTestCase {
            description: "projection star without from remains uncontrolled",
            sql: "SELECT *",
            rule: "SQBRSQL021",
            expected_anchor: Some("*"),
            expected_replacement: None,
        ...
        test_types::AdditionalLintRuleTestCase {
            description: "mixed dynamic pivot projection remains uncontrolled",
            sql: "SELECT *, customer_id FROM orders PIVOT(MAX(amount) FOR category IN (ANY))",
            rule: "SQBRSQL021",
            expected_anchor: Some("*"),
            expected_replacement: None,
        },
        test_types::AdditionalLintRuleTestCase {
            description: "static pivot star remains uncontrolled",
            sql: "SELECT * FROM orders PIVOT(MAX(amount) FOR category IN ('books'))",
            rule: "SQBRSQL021",
            expected_anchor: Some("*"),
            expected_replacement: None,
        },
        test_types::AdditionalLintRuleTestCase {
            description: "static pivot any string remains uncontrolled",
            sql: "SELECT * FROM orders PIVOT(MAX(amount) FOR category IN ('ANY'))",
            rule: "SQBRSQL021",
            expected_anchor: Some("*"),
            expected_replacement: None,
        },
        test_types::AdditionalLintRuleTestCase {
            description: "nested dynamic pivot does not exempt outer projection star",
            sql: "SELECT * FROM orders WHERE EXISTS (SELECT 1 FROM items PIVOT(MAX(amount) FOR category IN (ANY)))",
            rule: "SQBRSQL021",
            expected_anchor: Some("*"),
            expected_replacement: None,
        },
        test_types::AdditionalLintRuleTestCase {
            description: "unrelated any does not exempt static pivot star",
            sql: "SELECT * FROM orders PIVOT(MAX(amount) FOR category IN ('books')) WHERE customer_id = ANY (SELECT customer_id FROM customers)",
            rule: "SQBRSQL021",
            expected_anchor: Some("*"),
            expected_replacement: None,
        },
        test_types::AdditionalLintRuleTestCase {
            description: "comma joined dynamic pivot star remains uncontrolled",
            sql: "SELECT * FROM orders PIVOT(MAX(amount) FOR category IN (ANY)) AS p, customers AS c",
            rule: "SQBRSQL021",
            expected_anchor: Some("*"),
            expected_replacement: None,
        },
        test_types::AdditionalLintRuleTestCase {
            description: "joined audit relation parameter star remains uncontrolled",
            sql: "SELECT * FROM __sqlbuild_audit_parameter_0__ AS a CROSS JOIN customers AS c",
            rule: "SQBRSQL021",
            expected_anchor: Some("*"),
            expected_replacement: None,
        },
        ...
        test_types::AdditionalLintRuleTestCase {
            description: "dynamic pivot star has data-dependent columns",
            sql: "SELECT * FROM orders PIVOT(MAX(amount) FOR category IN (ANY ORDER BY category))",
            rule: "SQBRSQL021",
            expected_anchor: None,
            expected_replacement: None,
        },
        test_types::AdditionalLintRuleTestCase {
            description: "audit relation parameter star has invocation-dependent columns",
            sql: "SELECT * FROM __sqlbuild_audit_parameter_0__",
            rule: "SQBRSQL021",
            expected_anchor: None,
            expected_replacement: None,
        },
