...
impl HeaderParser {
    fn parse(tokens: Vec<HeaderToken>) -> Result<(AuthoredValue, Vec<HeaderColumnOffset>), String> {
        ...
        let value = if parser.peek().0 == END_TOKEN {
            ...
            AuthoredValue::Map(parser.parse_map(None, false, false, false, false, false)?)
        };
        parser.expect_end()?;
    ...
    fn parse_map(
        ...
        while !self.at_end_symbol(end) {
            ...
            if self.at_end_symbol(end) || self.peek().0 == END_TOKEN {
                return Err(format!(
                    "unexpected token '{key}' without a value; quote values with spaces"
                ));
            }
            if column_entries && record_column_entries {
                self.column_offsets
                    .push((key.clone(), key_token.2, key.chars().count()));
            }
            let value = if allow_outside && key == "outside" {
                self.parse_outside()?
            } else if matches!(key.as_str(), "pre_hooks" | "post_hooks") {
                self.parse_hook_field(&key)?
            } else if column_metadata && key == "type" {
                self.parse_type()?
            } else {
                self.parse_value(
                    key == "thresholds",
                    threshold_policy && matches!(key.as_str(), "warn" | "error"),
                    key == "columns",
                    column_entries,
                    end.is_none() && key == "columns",
                )?
            };
    ...
    fn parse_outside(&mut self) -> Result<AuthoredValue, String> {
        let lower = self.parse_value(false, false, false, false, false)?;
        if self.at_end_symbol(Some(")")) {
            return Err("outside threshold requires lower and upper values".to_owned());
        }
        let upper = self.parse_value(false, false, false, false, false)?;
    ...
    fn parse_type(&mut self) -> Result<AuthoredValue, String> {
        ...
        if token.0 != WORD_TOKEN
            || self
                .tokens
                .get(self.index + 1)
                .is_none_or(|next| next.0 != SYMBOL_TOKEN || next.1 != "(")
        {
            return self.parse_value(false, false, false, false, false);
        }
        ...
        while depth > 0 {
            ...
            if parameter.0 == SYMBOL_TOKEN && parameter.1 == "(" {
                depth += 1;
            } else if parameter.0 == SYMBOL_TOKEN && parameter.1 == ")" {
                depth -= 1;
            }
        }
    ...
    fn parse_value(
        ...
        if token.0 == WORD_TOKEN {
            self.advance();
            if self.peek().0 == SYMBOL_TOKEN && self.peek().1 == "(" {
                self.advance();
                ...
                return Ok(AuthoredValue::Map(vec![(
                    token.1,
                    AuthoredValue::Map(self.parse_map(
                        Some(")"),
                        threshold_policy,
                        allow_outside,
                        false,
        ...
        if self.match_symbol("(") {
            return Ok(AuthoredValue::Map(self.parse_map(
                Some(")"),
                threshold_policy,
                allow_outside,
                nested_columns,
                nested_metadata,
                record_nested_columns,
            )?));
        }
    ...
    fn parse_sequence(&mut self, end: &str) -> Result<Vec<AuthoredValue>, String> {
        ...
        while !self.at_end_symbol(Some(end)) {
            ...
            values.push(self.parse_value(false, false, false, false, false)?);
            self.match_symbol(",");
        }
    ...
    fn parse_hook_kwargs(&mut self) -> Result<Vec<(String, AuthoredValue)>, String> {
        ...
        while self.match_symbol(",") {
            ...
            self.consume_symbol(":")?;
            values.push((key, self.parse_value(false, false, false, false, false)?));
        }
        self.consume_symbol(")")?;
        Ok(values)
    }
